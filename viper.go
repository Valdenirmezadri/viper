...
type Viper struct {
	...
	_override           map[string]interface{}
...
func (p *Viper) setConfig(c map[string]interface{}) {
	p.mu.Lock()
	defer p.mu.Unlock()
	p._config = c
}
func (p *Viper) config() map[string]interface{} {
	p.mu.Lock()
	defer p.mu.Unlock()
	c := make(map[string]interface{})
	for k, v := range p._config {
		c[k] = v
	}
	return c
}

func (p *Viper) setOverride(c map[string]interface{}) {
	p.mu.Lock()
	defer p.mu.Unlock()
	p._override = c
}
func (p *Viper) override() map[string]interface{} {
	p.mu.Lock()
	defer p.mu.Unlock()
	c := make(map[string]interface{})
	for k, v := range p._override {
		c[k] = v
	}
	return c
}
...
func New() *Viper {
	...
	v.fs = afero.NewOsFs()
	v._config = make(map[string]interface{})
	v._override = make(map[string]interface{})
...
func (v *Viper) Sub(key string) *Viper {
	...
	if reflect.TypeOf(data).Kind() == reflect.Map {
		subv.setConfig(cast.ToStringMap(data))
		return subv
	}
	return nil
}
...
func (v *Viper) ReadConfig(in io.Reader) error {
	v.setConfig(make(map[string]interface{}))
